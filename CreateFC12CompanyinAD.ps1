...
$coPrice=300
$ouPath="OU=FCLT12,OU=FCLiteClients,DC=isllc,DC=com" 
$Global:ok=0
...
if ($Global:ok -eq 2) {
    ...
    Add-ADGroupMember "CN=FCLITE12,$($ouPath)" –Member "CN=$($coNum) $($companyName),OU=$($coNum)-$($companyName),$($ouPath)" #Adds company group to FCLite12.
    ...
    #creates fc## on common
    $user='isllc\fclitesmb'
    $pass = Get-Content "C:\scripts\temppass.txt" | ConvertTo-SecureString
    $credential = New-Object System.Management.Automation.PsCredential($user, $pass)
    
    New-PSDrive -Name 'common' -PSProvider "FileSystem" -Root '\\fclite12\common' -Credential $credential
    mkdir "\\fclite12\common\fc$($coNum)"
    mkdir "\\fclite12\common\fc$($coNum)\ups"
    Remove-PSDrive -Name 'common' 


    #Create WEBDAV virtual Directory with proper permissions on FCLITERDS10 (Pointing to FCLT12 server)
    Invoke-Command -ComputerName fcliterds10 -ScriptBlock { 

        New-WebVirtualDirectory -Site "Default Web Site" -Name "fc$($using:coNum)" -PhysicalPath "\\fclite12\common\fc$($using:coNum)\ups\" -force
